...
export function calculateFirstNonce(nonce) {
    return (((nonce - BigInt(1)) / MAX_NONCES_PER_ACCOUNT) * MAX_NONCES_PER_ACCOUNT +
        BigInt(1));
}
export function nonceAccount(nonce, sourceChain, messageTransmitterProgramId) {
    const srcDomain = sourceChain.toString();
    const usedNonces = findProgramAddress('used_nonces', messageTransmitterProgramId, [srcDomain, calculateFirstNonce(nonce).toString()]).publicKey;
    return usedNonces;
}
export async function createReceiveMessageInstruction(messageTransmitterProgramId, tokenMessengerProgramId, usdcAddress, circleMessage, attestation, payer) {
    ...
    const sourceUsdcAddress = new PublicKey(circleMessage.payload.burnToken.toUint8Array());
    const receiver = new SolanaAddress(circleMessage.payload.mintRecipient).unwrap();
    ...
    const localToken = findProgramAddress('local_token', tokenMessengerProgramId, [solanaUsdcAddress]);
    const remoteTokenMessengerKey = findProgramAddress('remote_token_messenger', tokenMessengerProgramId, [srcDomain]);
    const tokenPair = findProgramAddress('token_pair', tokenMessengerProgramId, [
        srcDomain,
        sourceUsdcAddress,
    ]);
    const custodyTokenAccount = findProgramAddress('custody', tokenMessengerProgramId, [solanaUsdcAddress]);
    const authorityPda = findProgramAddress('message_transmitter_authority', messageTransmitterProgramId, [tokenMessengerProgramId]).publicKey;
    ...
    const eventAuthority = findProgramAddress('__event_authority', messageTransmitterProgramId);
    const tokenMessengerEventAuthority = findProgramAddress('__event_authority', tokenMessengerProgramId);
    ...
    const messageTransmitterProgram = createMessageTransmitterProgramInterface(messageTransmitterProgramId);
