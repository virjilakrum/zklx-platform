...
function createTransferNativeWithPayloadInstruction(connection, tokenBridgeProgramId, wormholeProgramId, payer, message, from, mint, nonce, amount, targetAddress, targetChain, payload) {
    const methods = (0, program_js_1.createReadOnlyTokenBridgeProgramInterface)(tokenBridgeProgramId, connection).methods.transferNativeWithPayload(nonce, amount, Buffer.from(targetAddress), targetChain, Buffer.from(payload), null);
    // @ts-ignore
...
function getTransferNativeWithPayloadAccounts(tokenBridgeProgramId, wormholeProgramId, payer, message, from, mint, cpiProgramId) {
    const { wormholeBridge, wormholeMessage, wormholeEmitter, wormholeSequence, wormholeFeeCollector, clock, rent, systemProgram, } = sdk_solana_core_1.utils.getPostMessageCpiAccounts(tokenBridgeProgramId, wormholeProgramId, payer, message);
    return {
        ...
        custody: (0, index_js_1.deriveCustodyKey)(tokenBridgeProgramId, mint),
        authoritySigner: (0, index_js_1.deriveAuthoritySignerKey)(tokenBridgeProgramId),
        custodySigner: (0, index_js_1.deriveCustodySignerKey)(tokenBridgeProgramId),
        wormholeBridge,
        ...
        clock,
        sender: new web3_js_1.PublicKey(cpiProgramId === undefined ? payer : (0, index_js_1.deriveSenderAccountKey)(cpiProgramId)),
        rent,
        systemProgram,
