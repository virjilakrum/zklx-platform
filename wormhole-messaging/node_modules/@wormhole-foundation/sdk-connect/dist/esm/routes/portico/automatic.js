...
import { PorticoBridge, TokenTransfer, TransferState, Wormhole, amount, canonicalAddress, chainToPlatform, contracts, isAttested, isNative, isSourceInitiated, resolveWrappedToken, signSendWait, } from "./../../index.js";
...
export class AutomaticPorticoRoute extends AutomaticRoute {
    ...
    static async supportedDestinationTokens(sourceToken, fromChain, toChain) {
        ...
        const redeemTokenDetails = filters.byAddress(toChain.config.tokenMap, canonicalAddress(redeemToken));
        // Find the local/native version of the same token by symbol
        const locallyRedeemable = (filters.bySymbol(toChain.config.tokenMap, redeemTokenDetails.symbol) ?? [])
            .filter((td) => {
            return !td.original;
        })
            .map((td) => {
