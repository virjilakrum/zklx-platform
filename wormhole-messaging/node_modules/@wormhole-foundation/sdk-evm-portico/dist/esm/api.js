import { PorticoBridge, canonicalAddress, contracts, encoding, isEqualCaseInsensitive, isNative, nativeChainIds, resolveWrappedToken, toChainId, } from '@wormhole-foundation/sdk-connect';
...
export class PorticoApi {
    ...
    static async createOrder(network, chain, receiver, token, amount, destToken, quote, nonce) {
        try {
            ...
            const [isStartTokenNative, startToken] = resolveWrappedToken(network, chain, token);
            const [isFinalTokenNative, finalToken] = resolveWrappedToken(network, receiver.chain, destToken);
            const startTokenAddress = canonicalAddress(startToken);
            const finalTokenAddress = canonicalAddress(finalToken);
            const sourcePorticoAddress = contracts.portico.get(network, chain).portico;
            const destinationPorticoAddress = contracts.portico.get(network, receiver.chain).portico;
            const startingChainId = nativeChainIds.networkChainToNativeChainId.get(network, chain);
            const destinationChainId = nativeChainIds.networkChainToNativeChainId.get(network, receiver.chain);
            // Create the order
            const orderRequest = {
            ...
            const response = await axios.post(CREATE_ORDER_API_URL, orderRequest);
    ...
    static validateCreateOrderResponse = (response, request) => {
        if (!isEqualCaseInsensitive(request.porticoAddress || '', response.transactionTarget)) {
            throw new Error('portico address mismatch');
        }
        const decoded = porticoAbi.decodeFunctionData('start', response.transactionData);
        ...
        const [_, expectedChain] = nativeChainIds.platformNativeChainIdToNetworkChain('Evm', BigInt(request.destinationChainId));
        ...
        if (!isEqualCaseInsensitive(destinationPorticoAddress, request.destinationPorticoAddress || '')) {
