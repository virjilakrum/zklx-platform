...
function calcItemSize(item, data) {
    switch (item.binary) {
        ...
        case "bytes": {
            ...
            if ("layout" in item) {
                const { custom } = item;
                const layoutSize = internalCalcLayoutSize(item.layout, custom === undefined
                    ? data
                ...
                return lengthSize + (0, utils_js_1.checkItemSize)(item, layoutSize);
            }
            const { custom } = item;
            if ((0, utils_js_1.isBytesType)(custom))
                return lengthSize + custom.length; //assumed to equal item.size if it exists
            if ((0, utils_js_1.isFixedBytesConversion)(custom))
                return lengthSize + custom.from.length; //assumed to equal item.size if it exists
            if (custom === undefined)
                return data ? lengthSize + (0, utils_js_1.checkItemSize)(item, data.length) : null;
            return data !== undefined ? lengthSize + (0, utils_js_1.checkItemSize)(item, custom.from(data).length) : null;
        }
        case "array": {
        ...
        case "switch": {
            if (data !== undefined) {
                const [_, layout] = (0, utils_js_1.findIdLayoutPair)(item, data);
