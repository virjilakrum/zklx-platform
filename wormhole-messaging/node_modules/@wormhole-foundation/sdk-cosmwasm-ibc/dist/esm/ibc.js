...
import { chainToPlatform, encoding, isIbcMessageId, isIbcTransferInfo, isNative, toChainId, } from "@wormhole-foundation/sdk-connect";
import { CosmwasmAddress, CosmwasmPlatform, CosmwasmUnsignedTransaction, Gateway, IBC_MSG_TYPE, IBC_PACKET_DATA, IBC_PACKET_DST, IBC_PACKET_DST_PORT, IBC_PACKET_RECEIVE, IBC_PACKET_SEND, IBC_PACKET_SEQ, IBC_PACKET_SRC, IBC_PACKET_SRC_PORT, IBC_TIMEOUT_MILLIS, IBC_TRANSFER_PORT, computeFee, networkChainToChannels, } from "@wormhole-foundation/sdk-cosmwasm";
import { CosmwasmWormholeCore } from "@wormhole-foundation/sdk-cosmwasm-core";
const millisToNano = (seconds) => seconds * 1_000_000;
export class CosmwasmIbcBridge {
    network;
    chain;
    rpc;
    contracts;
    ...
    constructor(network, chain, rpc, contracts) {
        ...
        const channels = networkChainToChannels.get(network, chain) ?? {};
    ...
    async *transfer(sender, recipient, token, amount) {
        ...
        let recipientAddress = encoding.b64.encode(chainToPlatform(recipient.chain) === "Cosmwasm"
    ...
    async lookupMessageFromIbcMsgId(msg) {
        ...
        if (!tx)
            return null;
        return CosmwasmWormholeCore.parseWormholeMessageId(Gateway.chain, Gateway.coreAddress(this.network), tx);
    }
    ...
    async fetchTransferInfo(tx) {
        ...
        for (const xfer of xfers) {
            ...
            try {
                await qc.ibc.channel.packetCommitment(IBC_TRANSFER_PORT, xfer.id.srcChannel, xfer.id.sequence);
                xfer.pending = true;
            }
            catch (e) {
    ...
    parseIbcTransferInfo(tx) {
        // Try to get all IBC packets (sent/received)
        const packets = tx.events.filter((ev) => ev.type === IBC_PACKET_SEND || ev.type === IBC_PACKET_RECEIVE);
        if (packets.length === 0)
        ...
        for (const packet of packets) {
            ...
            for (const attr of packet.attributes) {
                // set on msgId
                if (attr.key === IBC_PACKET_SRC)
                    msgId.srcChannel = attr.value;
                if (attr.key === IBC_PACKET_DST)
                    msgId.dstChannel = attr.value;
                if (attr.key === IBC_PACKET_SEQ)
                    msgId.sequence = Number(attr.value);
                if (attr.key === IBC_PACKET_SRC_PORT)
                    msgId.srcPort = attr.value;
                if (attr.key === IBC_PACKET_DST_PORT)
                    msgId.dstPort = attr.value;
                // set on the xfer obj
                if (attr.key === IBC_PACKET_DATA)
